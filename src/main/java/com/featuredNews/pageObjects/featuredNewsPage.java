...
public class featuredNewsPage extends BasePage {
	...
	@FindBy(xpath="((//*[@class='ControlZone--control'])[8]//div[@id='QuicklinksItemTitle'])")
	...
	Locators loc=new Locators(BaseClass.getDriver());  				//creating an instance for locators 
	...
	public void verifyTooltip() {		
	    ...
	    {	    	
	    	String headline=toolTip.get(i).getText();       		 //storing title of news in "headline"
			String tooltip=toolTip.get(i).getAttribute("title");	 //storing toolTip of news in "tooltip"
	...
	public void getNewsInformation() throws IOException, InterruptedException {
		...
		for(int i=0;i<newsList.size();i++) {
			 ...
			 //info=newsText.getText();                           	//getting content of each news and storing it into "info"
			// System.out.println(info);								//printing information of each news
			 
			 
			 Thread.sleep(8000);
			 List<WebElement> s1=driver.findElements(By.xpath("//div[@data-sp-feature-tag='Rich Text Editor']"));
			 if(s1.size()==1)
			 {
			 	System.out.println(s1.get(0).getText()+"\n");
			 	info=s1.get(0).getText();
	...
	//verifying  apps and tools block is displayed or not in home page
